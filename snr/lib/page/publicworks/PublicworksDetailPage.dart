...
class _PublicworksDetailPageState extends State<PublicworksDetailPage> with AutomaticKeepAliveClientMixin<PublicworksDetailPage>, MyListState<PublicworksDetailPage>, WidgetsBindingObserver {
  ///snr設定檔
  var config;
  ///現在按鈕enum
  var nowType = buttonType.inst;
  ///hub
  var strHub = "";
  ///現在功能
  var typevalue = "1";
  ...
  ///跳轉工單號
  final List<String> toTransWkNoArray = [];
  ///跳轉年月
        ...
        children: <Widget>[
          ButtonTheme(
            ...
            child: new MyToolButton(
              ...
              textColor: nowType == buttonType.inst ? Colors.red : Colors.grey[700],
              onPress: () {
                if (isLoading) {
          ...
          ButtonTheme(
            ...
            child: new MyToolButton(
              ...
              shape: new RoundedRectangleBorder(
                  ...
                  side: BorderSide(color: Colors.grey)),
              text: CommonUtils.getLocale(context)
                  .text_maintain + "-${maintainCount}",
              color: Color(MyColors.hexFromStr("#f0fcff")),
              fontSize: MyScreen.normalListPageFontSize(context),
              textColor: nowType == buttonType.maintain ? Colors.red : Colors.grey[700],
              onPress: () {
                if (isLoading) {
                  Fluttertoast.showToast(msg: CommonUtils.getLocale(context).loading_text);
                  return;
          ...
          ButtonTheme(
            ...
            child: new MyToolButton(
              ...
              shape: new RoundedRectangleBorder(
                  ...
                  side: BorderSide(color: Colors.grey)),
              text: CommonUtils.getLocale(context)
                  .text_instFix + "-${instFixCount}",
              color: Color(MyColors.hexFromStr("#fafff2")),
              fontSize: MyScreen.normalListPageFontSize(context),
              textColor: nowType == buttonType.instFix ? Colors.red : Colors.grey[700],
              onPress: () {
                if (isLoading) {
                  Fluttertoast.showToast(msg: CommonUtils.getLocale(context).loading_text);
                  return;
          ...
          ButtonTheme(
            ...
            child: new MyToolButton(
              ...
              shape: new RoundedRectangleBorder(
                  ...
                  side: BorderSide(color: Colors.grey)),
              text: CommonUtils.getLocale(context)
                  .text_fixfix + "-${fixfixCount}",
              color: Color(MyColors.hexFromStr("#fef5f6")),
              fontSize: MyScreen.normalListPageFontSize(context),
              textColor: nowType == buttonType.fixfix ? Colors.red : Colors.grey[700],
              onPress: () {
                if (isLoading) {
                  Fluttertoast.showToast(msg: CommonUtils.getLocale(context).loading_text);
                  return;
  ...
  getApiDataList() async {
    var res = await PublicworksDao.getQueryWorkWarning(_getStore(),
    city: strCity,
    sort: strSort,
    hub: strHub,
    type: typevalue
  ...
  /// call 跳轉api
  postTransferAPI(to, custCDList) async {
  ...
  /// call 工異api
  postTransferPublicworksAPI(to, memo, targetMan, jsonCustCD,) async {
    await DefaultTableDao.didTransferPublicwork(context,
    to: to,
    from: typeof,
    memo: memo,
    accNo: user.accNo,
    ...
    List<Widget> wList = [];
      switch (nowType) {
        case buttonType.inst: 
          sortArray = ["正常","可異","派修","追蹤"];
          break;
        case buttonType.maintain: 
          sortArray = ["正常","可異","派修","追蹤"];
          break;
        case buttonType.instFix:
          if (user.isTransFromDeduct == 1 && this.toTransformArray.length < 2) {
            sortArray = ["正常","追蹤","扣點(待確認)"];
          }
          else {
            sortArray = ["正常","追蹤"];
          }
        ...
        showDialog(
          ...
          builder: (context) {
            var dialog = CupertinoAlertDialog(
              ...
              actions: <Widget>[
                CupertinoTextField(
                  controller: TextEditingController(text: toTargetManArray[0]),
                  prefix: Text('目標: ', style: TextStyle(color: Colors.black, fontSize: MyScreen.normalListPageFontSize(context))),
                  autofocus: true,
                  decoration: BoxDecoration(
                    borderRadius: BorderRadius.circular(5.0),
                    border: Border.all(color: Colors.grey, style: BorderStyle.solid)
                ...
                CupertinoButton(
                  onPressed: (){
                    if (targetMan == null) {
                      Fluttertoast.showToast(msg: '目標人員為必填唷！');
                      return;
                    }
                    else {
                      targetMan = toTargetManArray[0];
                    }
                    if (memoText == "") {
                      Fluttertoast.showToast(msg: '備註為必填唷！');
                      return;
                    }
                    Map<String, dynamic> para = new Map<String, dynamic>();
                    Map<String, dynamic> para2 = new Map<String, dynamic>();
                    for (var i = 0; i< toTransformArray.length; i++) {
                      para.addAll({"CustCD": "${toTransformArray[i]}"});
                      para.addAll({"WorkNo": "${toTransWkNoArray[i]}"});
                      para.addAll({"Comment": "${memoText}"});
                      para.addAll({"Target": "${targetMan}"});
                      para.addAll({"YearMonth": "${toTransYearMonthArray[0]}"});
                      para2.addAll({"${i}": para});
                    }
                   
                    var jsonCustCD = json.encode(para2);
                ...
                CupertinoButton(
                  onPressed: (){
                    ...
                    if (typevalue == "3" || typevalue == "4") {
                      ...
                      postTransferAPI(to, jsonCustCD);
                      Navigator.pop(context);
                    } 
                    else {
                      postTransferAPI(to, toTransformArray);
                      Navigator.pop(context);
                    }
  ...
  ///跳轉function
  void _addTransform({String custNo, String workMan, String wkNo, String n_coid, String target, String yearMonth}) {
    setState(() {
      ...
      if (toTransformArray.contains(custNo)) {
        ...
        if (typevalue == "1" || typevalue == "2") {
          ...
          toTargetManArray.removeAt(index);
          toTransWkNoArray.removeAt(index);
        }
        else {
          var index = toTransformArray.indexOf(custNo);
          toTransformArray.removeAt(index);
          toTargetManArray.removeAt(index);
          toTransWkNoArray.removeAt(index);
          toTransYearMonthArray.removeAt(index);
        }
      }
      else {
        ...
        if (typevalue == "1" || typevalue == "2") {
           ...
           toTargetManArray.add(workMan);
           toTransWkNoArray.add(n_coid);
        }
        else {
          toTransformArray.add(custNo);
          toTargetManArray.add(workMan);
          toTransWkNoArray.add(n_coid);
          toTransYearMonthArray.add(yearMonth);
        }
      }
    });
  }
 ...
 ///小ping dialog
  ...
  Future<Null> handleRefresh() async {
    ...
    if (res != null && res.result) {
      ...
      setState(() {
        ...
        toTargetManArray.clear();
        toTransWkNoArray.clear();
        toTransYearMonthArray.clear();
        pullLoadWidgetControl.dataList.clear();
        instCount = res.data["ISNT"] ?? "0";
        maintainCount = res.data["MAINTAIN"] ?? "0";
        instFixCount = res.data["INSTFIX"] ?? "0";
        fixfixCount = res.data["FIXFIX"] ?? "0";
  ...
  void didChangeDependencies() {
    var list = _getStore().state.publicWorksList;
                ...
                Expanded(
                  child: Row(
                    ...
                    children: <Widget>[
                      ...
                      SizedBox(),
                      Text(CommonUtils.getLocale(context).text_ww, style: TextStyle(fontSize: MyScreen.normalPageFontSize(context), color: Colors.yellow)),
                      SizedBox(),
                      SizedBox(),
              ...
              child: new Row(
                ...
                children: <Widget>[
                  ...
                  ButtonTheme(
                    ...
                    child: new MyToolButton(
                      padding: EdgeInsets.only(left: 10.0, right: 10.0),
                      text: CommonUtils.getLocale(context).text_bp,
